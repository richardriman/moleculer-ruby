...
RSpec.describe Moleculer::Registry::ItemCatalog do
  ...
  describe "#register_items_for_node" do
    let(:test_service_handler) { Moleculer::Service::Action.new(service.new(broker, node), "test-service-handler", nil, {}) }
    let(:service) do
    ...
    let(:other_test_service_handler) { Moleculer::Service::Action.new(service.new(broker, node), "other-test-service-handler", nil, {}) }
